...
def launch_stack(ctx, stack, ssh_key, x86_ami, arm_ami):
    ...
    env_vars = ' '.join(env)
    ctx.run(
        f"{env_vars} {prefix} inv -e system-probe.start-microvms {provision} --kmt-working-directory {get_kmt_os().kmt_dir} --instance-type-x86={X86_INSTANCE_TYPE} --instance-type-arm={ARM_INSTANCE_TYPE} --x86-ami-id={x86_ami} --arm-ami-id={arm_ami} --ssh-key-name={ssh_key} --infra-env=aws/sandbox --vmconfig={vm_config} --stack-name={stack} {local}"
    )
