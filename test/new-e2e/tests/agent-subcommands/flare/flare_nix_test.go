...
func TestLinuxFlareSuite(t *testing.T) {
	t.Parallel()
	e2e.Run(t, &linuxFlareSuite{}, e2e.FakeIntakeStackDef(e2e.WithAgentParams(agentparams.WithLatest()), e2e.WithVMParams(ec2params.WithOS(ec2os.UbuntuOS)), e2e.WithFakeIntakeParams(fakeintakeparams.WithoutLoadBalancer())))
}
