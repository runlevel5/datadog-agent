...
func createStatsPayload(agentEnv string, agentHost string,
	svc string, operation string, typ string, kind string,
	resource string, tracerHost string, env string,
	cid string, ctags []string, peerTags []string,
	root bool) *pb.StatsPayload {
	traceroot := pb.TraceRootFlag_TRUE
	if !root {
		traceroot = pb.TraceRootFlag_FALSE
	}
