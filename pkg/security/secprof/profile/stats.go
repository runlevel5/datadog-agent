...
type Stats struct {
	...
	addedCount     map[model.EventType]map[activitytree.NodeGenerationType]*atomic.Uint64
	droppedCount   map[model.EventType]map[activitytree.NodeDroppedReason]*atomic.Uint64
}
...
func NewActivityTreeNodeStats() *Stats {
	ats := &Stats{
		...
		addedCount:     make(map[model.EventType]map[activitytree.NodeGenerationType]*atomic.Uint64),
		droppedCount:   make(map[model.EventType]map[activitytree.NodeDroppedReason]*atomic.Uint64),
	}

	// generate counters
	for i := model.EventType(0); i < model.MaxKernelEventType; i++ {
		ats.processedCount[i] = atomic.NewUint64(0)
		ats.addedCount[i] = map[activitytree.NodeGenerationType]*atomic.Uint64{
			activitytree.Unknown:        atomic.NewUint64(0),
			activitytree.Runtime:        atomic.NewUint64(0),
			activitytree.Snapshot:       atomic.NewUint64(0),
			activitytree.ProfileDrift:   atomic.NewUint64(0),
			activitytree.WorkloadWarmup: atomic.NewUint64(0),
		}

		ats.droppedCount[i] = make(map[activitytree.NodeDroppedReason]*atomic.Uint64)
		for _, reason := range activitytree.AllDropReasons {
