...
func (pa patcherAdapter) GetRecommendations(ns string, ownerRef metav1.OwnerReference) (string, []datadoghq.DatadogPodAutoscalerContainerResources, error) {
	...
	if podAutoscalers[0].ScalingValues.Vertical != nil && podAutoscalers[0].ScalingValuesVersion != nil {
		return podAutoscalers[0].ScalingValuesHash, podAutoscalers[0].ScalingValues.Vertical.ContainerResources, nil
	}
