...
type PatcherAdapter interface {
	...
	GetRecommendations(ns string, ownerRef metav1.OwnerReference) ([]datadoghq.DatadogPodAutoscalerContainerResources, error)
}
...
// If no PodAutoscalerInternal is found, it returns (nil, nil)
func (pa patcherAdapter) GetRecommendations(ns string, ownerRef metav1.OwnerReference) ([]datadoghq.DatadogPodAutoscalerContainerResources, error) {
	...
	return podAutoscalers[0].ScalingValues.ContainerResources, nil
}
