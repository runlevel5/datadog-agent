...
func (s *USMHTTP2Suite) TestHTTP2ManyDifferentPaths() {
	...
	for batchIdx := 0; batchIdx < batches; batchIdx++ {
		...
		assert.Eventuallyf(t, func() bool {
			...
			return (repetitionsPerRequest*batchSize-1) <= matches.Load() && matches.Load() <= (repetitionsPerRequest*batchSize+1)
		}, time.Second*10, time.Millisecond*100, "%v != %v", &matches, repetitionsPerRequest*batchSize)
