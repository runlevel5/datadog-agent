...
static __always_inline void process_headers_frame(struct __sk_buff *skb, http2_stream_t *current_stream, skb_info_t *skb_info, conn_tuple_t *tup, dynamic_table_value_t *dynamic_table_value, struct http2_frame *current_frame_header, http2_telemetry_t *http2_tel) {
    const __u32 zero = 0;
...
static __always_inline void parse_frame(struct __sk_buff *skb, skb_info_t *skb_info, conn_tuple_t *tup, http2_stream_key_t *http2_stream_key, dynamic_table_value_t *dynamic_table_value, struct http2_frame *current_frame, http2_telemetry_t *http2_tel) {
    ...
    if (current_frame->type == kHeadersFrame) {
        process_headers_frame(skb, current_stream, skb_info, tup, dynamic_table_value, current_frame, http2_tel);
    }
...
int socket__http2_frames_parser(struct __sk_buff *skb) {
    ...
    for (__u8 index = 0; index < HTTP2_FRAMES_PER_TAIL_CALL; index++) {
        ...
        parse_frame(skb, &dispatcher_args_copy.skb_info, &dispatcher_args_copy.tup, http2_stream_key, dynamic_table_value, &current_frame.frame, http2_tel);
    }
