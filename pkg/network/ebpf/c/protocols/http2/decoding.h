...
#pragma unroll(HTTP2_MAX_PSEUDO_HEADERS_COUNT_FOR_FILTERING)
    for (__u8 headers_index = 0; headers_index < HTTP2_MAX_PSEUDO_HEADERS_COUNT_FOR_FILTERING; ++headers_index) {
        ...
        if (is_indexed) {
            ...
            parse_field_indexed(current_stream, dynamic_index, index, *global_dynamic_counter, http2_tel);
            continue;
        }
...
#pragma unroll(HTTP2_MAX_HEADERS_COUNT_FOR_PROCESSING)
    for (__u8 iteration = 0; iteration < HTTP2_MAX_HEADERS_COUNT_FOR_PROCESSING; ++iteration) {
        ...
        if (is_path_index(current_header->original_index)) {
            current_stream->path.dynamic_table_entry = dynamic_table_value->key.index;
            current_stream->path.temporary = dynamic_table_value->temporary;
        } else if (is_status_index(current_header->original_index)) {
            current_stream->status_code.dynamic_table_entry = dynamic_table_value->key.index;
            ...
            current_stream->request_method.dynamic_table_entry = dynamic_table_value->key.index;
...
static __always_inline void process_headers_frame(struct __sk_buff *skb, http2_stream_t *current_stream, skb_info_t *skb_info, conn_tuple_t *tup, dynamic_table_value_t *dynamic_table_value, http2_frame_t *current_frame_header, http2_telemetry_t *http2_tel) {
    ...
    __u8 interesting_headers = filter_relevant_headers(skb, skb_info, tup, current_stream, &dynamic_table_value->key, headers_to_process, current_frame_header->length, http2_tel);
...
int socket__http2_eos_parser(struct __sk_buff *skb) {
    ...
    for (__u16 index = 0; index < HTTP2_MAX_FRAMES_FOR_EOS_PARSER_PER_TAIL_CALL; index++) {
        ...
        if (is_rst && ((current_stream->status_code.dynamic_table_entry == current_stream->status_code.static_table_entry) || current_stream->request_started == 0)) {
